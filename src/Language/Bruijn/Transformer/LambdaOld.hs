...
transformFreestanding :: TermAnn -> TermAnn -> TermAnn -> TermAnn
transformFreestanding term sub next = do
...
transformDefinition name term sub next = do
  let term' = trace ("--------" <> show name) (transformSub sub term)
...
transformSubstitution = mapTermAnn $ \case
  AnnF a (DoF term sub next) -> transformNext "" term next
...
transformTerm = foldFix $ \case
  ...
  -- the final "next" will always remain in subs (TODO?)
  AnnF a (DefinitionF _ term _ _) -> term
...
transformMain (Fix (AnnF a (PreprocessorF _ _ next))) = transformMain next
transformMain (Fix (AnnF a (DoF _ _ next))) = transformMain next
